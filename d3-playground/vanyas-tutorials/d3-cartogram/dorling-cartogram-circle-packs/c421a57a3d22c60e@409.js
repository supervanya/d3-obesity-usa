...
`
  ...
  main.variable(observer()).define(["map5"], function(map5){return(
map5.cloneNode(true)
)});
  main.variable(observer("legend")).define("legend", ["d3","DOM","color"], function(d3,DOM,color)
{
...
`
  ...
  main.variable(observer("map1")).define("map1", ["createBaseMap"], function(createBaseMap){return(
createBaseMap()
)});
  main.variable(observer()).define(["md"], function(md){return(
...
`
  ...
  main.variable(observer("map2")).define("map2", ["createBaseMap","d3","states","NODE"], function(createBaseMap,d3,states,NODE)
{
...
`
  ...
  main.variable(observer("createStatePacks")).define("createStatePacks", ["radius","packSiblings","packEnclose","states"], function(radius,packSiblings,packEnclose,states){return(
(data) => {
...
md`Below is a rendering of each state pack circle, positioned using its centroid coordinates. Notice the overlap between pack circles which will be addressed in the next step:`
  ...
  main.variable(observer("map3")).define("map3", ["createBaseMap","d3","createStatePacks","dataByState"], function(createBaseMap,d3,createStatePacks,dataByState)
{
  const map = createBaseMap();
...
`
  ...
  main.variable(observer("applySimulation")).define("applySimulation", ["d3","width","height","NODE"], function(d3,width,height,NODE){return(
(nodes) => {
  ...
  main.variable(observer("map4")).define("map4", ["createBaseMap","createStatePacks","dataByState","applySimulation","d3"], function(createBaseMap,createStatePacks,dataByState,applySimulation,d3)
{
  const map = createBaseMap();
  const statesPacked = createStatePacks(dataByState);
...
`
  ...
  main.variable(observer("map5")).define("map5", ["createBaseMap","d3","createStatePacks","dataByState","applySimulation","color","withBroadbandPct"], function(createBaseMap,d3,createStatePacks,dataByState,applySimulation,color,withBroadbandPct)
{
  const map = createBaseMap()
  const svg = d3.select(map);
  const statesPacked = createStatePacks(dataByState);
...
{ MIN_RADIUS: 2.5, MAX_RADIUS: 20, PADDING: 2 }
  ...
  main.variable(observer("packSiblings")).define("packSiblings", ["d3"], function(d3){return(
(values) => d3.packSiblings(values)
)});
  main.variable(observer("packEnclose")).define("packEnclose", ["d3"], function(d3){return(
(nodes) => d3.packEnclose(nodes)
)});
  main.variable(observer("scheme")).define("scheme", ["d3"], function(d3){return(
d3.schemeGnBu
)});
  main.variable(observer("color")).define("color", ["d3","scheme"], function(d3,scheme){return(
d3.scaleQuantize()
  .domain([0, 1])
  .range(scheme[9])
)});
  main.variable(observer("radius")).define("radius", ["d3","data","NODE"], function(d3,data,NODE){return(
d3.scaleSqrt()
  .domain(d3.extent(data, d => d.total_households))
  .range([NODE.MIN_RADIUS, NODE.MAX_RADIUS])
)});
  main.variable(observer("geoPath")).define("geoPath", ["d3"], function(d3){return(
d3.geoPath()
)});
  main.variable(observer("createBaseMap")).define("createBaseMap", ["d3","height","stateBoundaries","geoPath","nation"], function(d3,height,stateBoundaries,geoPath,nation){return(
() => {  
  ...
  main.variable(observer("withBroadbandPct")).define("withBroadbandPct", function(){return(
d => {
...
`
  ...
  main.variable(observer("dataByState")).define("dataByState", ["d3","data"], function(d3,data){return(
d3.group(data, d => d.state)
)});
  main.variable(observer("data")).define("data", ["d3","parseRow"], function(d3,parseRow){return(
d3.csv("https://gist.githubusercontent.com/clhenrick/5fe3463586ee196820ac74a62fd0445a/raw/09422f3803f59e5530836b1614774c5a7a9381f3/acs5_2017_internet_access_joined_rural_pct_names.csv", parseRow)
  ...
  main.variable(observer("parseRow")).define("parseRow", function(){return(
(row) => {
  for (let key in row) {
    if (key === "name") {
  ...
  main.variable(observer("nation")).define("nation", ["topojson","us"], function(topojson,us){return(
topojson.mesh(us, us.objects.nation)
)});
  main.variable(observer("states")).define("states", ["topojson","us","geoPath"], function(topojson,us,geoPath)
{
  ...
  main.variable(observer("stateBoundaries")).define("stateBoundaries", ["topojson","us"], function(topojson,us){return(
topojson.mesh(us, us.objects.states, (a, b) => a !== b)
)});
  main.variable(observer("counties")).define("counties", ["topojson","us"], function(topojson,us){return(
